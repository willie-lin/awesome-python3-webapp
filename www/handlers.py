...
COKKIE_NAME = 'awesession'
_COOKIE_KEY = configs.session.secret


def user2cookie(user,max_age):
    '''
    Geneate cookie str by user
    :param user:
    :param max_age:
    :return:
    '''
    ...
    L = [user.id,expires,hashlib.sha1(s.encode('uthf-8')).hexdigest()]
    return '-'.join(L)
...
def signout(request):
    ...
    r.set_cookie(COKKIE_NAME,'-deleted-',max_age=0,httponly=True)
...
def api_register_user(*,email,name,passwd):
    ...
    #make session cookie:
    r = web.Response()
    r.set_cookie(COKKIE_NAME,user2cookie(user,886400),max_age=86400,httponly=True)
    user.passwd = '******'
    r.content_type = 'application/json'
    r.body = json.dumps(user,ensure_ascii=False).encode('utf-8')
    return r
